...
<c:choose>
    <c:when test="${shoppingcart==null}"><div>
          </div><div><br/>
          <br/>
                  </div>
                   <div>       
               </div><div class="col-md-12">
<h3>购物车是空的,<a href="selectAllBook.do?id=2">加点东西吧</a>
<br/><br/><br/><br/><br/><br/>
<br/></h3>
</div>
</c:when>
<c:when test="${shoppingcart!=null}">
 	...
 	<c:set var="sum" value="0"></c:set>   <!-- 总价 -->
   <table>
                     <tr>
                        <th></th>
                        <th>Book_name</th>
                     ...
                     <c:forEach items="${shoppingcart}" var="cart" varStatus="state">
                     <tr class="${state.index}">
                     
                        <td><img width="70" height="100"  src="${cart.book.book_img}" alt="${cart.book.book_id}"></td>
                        <td>${cart.book.book_name}</td>
                        
                        <td><strong class="pri">${cart.book.book_price}</strong></td>  
                        <td>
                        
						<input type="number" class="num" name="count" min="1" max="999" value="${cart.count}"/>
						<input type="hidden" id="book_id" value="${cart.book.book_id}">
                    	 </td>
                        <td><strong class="price" id="price"><c:set var="a" value="${cart.book.book_price}"/>
					<c:set var="b" value="${cart.count}"/>
					<c:set var="c" value="${a*b}"/>
					${c}</strong></td>
					 <c:set var="sum" value="${sum+c}"></c:set>
					
                        <td> 
                        <span class="red">         	
                       		 <a href="deleteCart.do?book_id=${cart.book.book_id}" id="del" ><i class="fa fa-times"  aria-hidden="true" ></i></a></span>
                        </td>
                     </tr>
                  ...
                  <div class="col-sm-6 col-md-6">
                     <a href="selectAllBook.do?id=2" class="button red">CONTINUE SHOPPING</a>
                  </div>
                  ...
                  <div class="col-sm-4 col-md-4">
                     <div class="shipping-outer">
                        <h2>Cart totals</h2>
                        <form action="savaShoppingCart.do" method="post">
                        <ul>
          
                           <li ><input type="text" style="font-size:50px;color:#F00;border:0px;text-align:center;" name="sum" id="pricesum" value="${sum}" readonly/></li>
                           <li class="text-center">
                           
                              <input type="submit" class="redbutton" value="提交订单"/>
							
                           </li>
                           </form>
                           <li><input  name="btnq" id="btnq" class="btn btn-secondary"  type="button" value="删除购物车"></li>
                        </ul>
